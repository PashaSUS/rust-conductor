...
impl WorkflowEngine {
    ...
    pub async fn sweep_orphaned_tasks(&self) -> Result<u64, EngineError> {
        ...
        for shard in self.shards.all_shards() {
            ...
            // 3. Sweep ACTUALLY-stale SUB_WORKFLOW parent tasks.
            //    The hot path `try_complete_parent_sub_workflow` advances the
            //    parent the moment a child completes, so any SUB_WORKFLOW task
            //    that is still IN_PROGRESS but hasn't been touched in a while
            //    is the genuine "child finished but parent never got the
            //    notification" case (e.g. the backend that completed the child
            //    crashed before advancing the parent).
            //
            //    Without this time filter every parent with an active child
            //    is picked up on every cycle, calls `advance_workflow` for
            //    nothing, and floods the DB at high throughput.
