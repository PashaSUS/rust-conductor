...
export const DEFAULT_COLOR = { bg: "#f9fafb", border: "#d1d5db", text: "#374151" };
...
export function TaskNode({ data }: { data: { label: string; taskType: string; status?: string; refName: string } }) {
...
export function buildGraph(
  ...
  if (contentNodes.length > 0) {
    ...
    const maxXVal = Math.max(...contentNodes.map((n) => n.position.x));
    const centerX = (minX + maxXVal) / 2;
